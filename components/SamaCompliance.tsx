...
export function SamaExecutive(){
  ...
  return <div className="sama-module">
    <div className="sama-hero"><div><p className="eyebrow">SAMA CSF / integrated assessment</p><h1>Compliance position</h1><p>Control-level assessment from the authoritative Muhlah SAMA CSF self-assessment workbook. Recorded maturity remains separate from validated closure.</p><div className="source-chip"><Database size={14}/>249 controls · 35 subdomains · 4 domains</div></div><div className="score-ring" style={{"--score":`${s.completion*3.6}deg`} as React.CSSProperties}><div><strong>{s.completion}%</strong><span>workstream score</span><small>Not a compliance certification</small></div></div></div>
    <div className="sama-kpis">{[
      ["Total controls",controls.length,"Workbook source"],["Confirmed closed",s.counts.Completed,"Workbook status field"],["Recorded ML3",controls.filter(c=>c.sourceMaturity===3).length,"Validate evidence"],["Recorded ML2",controls.filter(c=>c.sourceMaturity===2).length,"Maturity gap remains"],["Dependencies / approval",s.counts["Blocked / Waiting for Approval"],"Reported as dependencies"],["Not applicable",s.counts["Not Applicable"],"Workbook source"],["Recorded ML1",controls.filter(c=>c.sourceMaturity===1).length,"Priority maturity gap"],["Evidence referenced",s.evidenceReady,"Links still require validation"]
...
export function SamaMapping(){
  const capabilities=[
    ...
    ["Qualys","Implemented · NOC dependency","Vulnerability Management"],
