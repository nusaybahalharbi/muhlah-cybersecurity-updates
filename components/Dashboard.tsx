...
import { managementActions, vendors } from "@/data/dashboard";
...
const tabs = ["Executive overview", "SAMA & ML3", "Third-party risk", "Governance"] as const;
...
const statusMeta: Record<string, { label: string; className: string }> = {
  ...
  "Blocked / Waiting for Approval": { label: "Blocked", className: "risk" },
...
function Overview({ onChange }: { onChange: (tab: Tab) => void }) {
  const closed = controls.filter(control => control.status === "Completed").length;
  const inProgress = controls.filter(control => ["Partially Completed", "In Progress"].includes(control.status)).length;
  const evidenceRequired = controls.filter(control => control.status === "Requires Evidence / Verification").length;
  return <main>
    <section className="executive-hero">
      ...
      <div className="hero-progress"><div className="hero-progress-head"><span>Overall assessment progress</span><b>30%</b></div><ProgressBar value={30}/><div className="hero-progress-meta"><div><strong>ML3</strong><span>Target maturity</span></div><div><strong>70%</strong><span>Assessment remaining</span></div><div><strong>147</strong><span>Requirements tracked</span></div></div><button onClick={() => onChange("SAMA & ML3")}>Review SAMA position <ArrowUpRight size={16}/></button></div>
    </section>
    <section className="metric-grid"><MetricCard label="SAMA assessment" value="30%" detail="Evidence-weighted progress · target ML3" accent="primary"/><MetricCard label="Awareness coverage" value="100%" detail="All employees completed KnowBe4" accent="good"/><MetricCard label="Control workstream" value={`${inProgress}`} detail={`${closed} closed · ${evidenceRequired} require evidence`} accent="blue"/><MetricCard label="Management decisions" value={`${managementActions.length}`} detail="Approval, dependency or ownership items" accent="attention"/></section>
    <section className="executive-grid">
      ...
      <article className="card decision-card"><div className="card-title"><div><span>LEADERSHIP FOCUS</span><h2>Management attention</h2></div><b>{managementActions.length}</b></div><div className="decision-list">{managementActions.slice(0,4).map((item, index) => <div key={item.issue}><span className="decision-index">0{index + 1}</span><div><strong>{item.issue}</strong><p>{item.decision}</p></div><StatusPill status={item.status === "Blocked" ? "Blocked / Waiting for Approval" : item.status}/></div>)}</div></article>
    </section>
...
function SamaView() {
  const domains = useMemo(() => {
    ...
    return [...grouped].map(([name, rows]) => ({ name: name.replace("Cyber Security", "Cybersecurity"), total: rows.length, closed: rows.filter(row => row.status === "Completed").length, progress: rows.filter(row => ["Partially Completed", "In Progress"].includes(row.status)).length, remaining: rows.filter(row => !["Completed", "Partially Completed", "In Progress"].includes(row.status)).length, score: Math.round(rows.reduce((sum, row) => sum + row.completion, 0) / rows.length) }));
  }, []);
  const statusData = [
    { name: "Evidence required", value: controls.filter(c => c.status === "Requires Evidence / Verification").length, color: "#d69f43" },
    { name: "Partially completed", value: controls.filter(c => c.status === "Partially Completed").length, color: "#2777c7" },
    { name: "In progress", value: controls.filter(c => c.status === "In Progress").length, color: "#68a7db" },
    { name: "Not started / blocked", value: controls.filter(c => ["Not Started", "Blocked / Waiting for Approval"].includes(c.status)).length, color: "#8d99a6" },
  ];
  return <main>
    <SectionHeading eyebrow="SAMA CSF & ML3" title="Maturity and compliance trajectory" copy="Evidence-weighted reporting across all 147 requirements. Scores indicate assessment progress—not regulatory certification."/>
    <section className="sama-topline"><article><span>Assessment progress</span><strong>30%</strong><ProgressBar value={30}/><small>70% remains to complete the evidence-led assessment</small></article><article><span>Target maturity</span><strong>ML3</strong><p>Defined, approved and consistently implemented controls with demonstrable operating evidence.</p></article><article><span>Evidence position</span><strong>{statusData[0].value}</strong><p>Requirements currently awaiting control-specific evidence or verification.</p></article></section>
